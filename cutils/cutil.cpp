...
struct node  {
    ...
    long sum;
    node *left, *right;
    node(int _k) : count(1), key(_k), fix(rand()), sum(_k) { left=right=0; }
};
...
void L1partition_approx(int *hist, int n1, int* x, int n, double epsilon, double ratio, long seed)    {
    ...
    while ( len <= n )   {
        ...
        for (int i=len-1; i<n; i++) {
            ...
            score[i][off] = (score[i][off] < 0) ? 0: score[i][off];
            delete loc;
            if ( i < n-1 )  {
                sum += x[i+1] - x[i-len+1];
                insert(root, x[i+1]);
...
void L1partition(int *hist, int n1, int* x, int n, double epsilon, double ratio, long seed)    {
    ...
    for (int i=0; i<n; i++) {
        ...
        for (int j=0; j<n-i; j++)   {
            ...
            score[i][j] = (score[i][j] < 0)? 0 : score[i][j];
        }
    }
